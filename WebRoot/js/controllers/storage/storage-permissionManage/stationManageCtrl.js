 ...
 */
define(['../../../app','../../../services/storage/storage-permissionManage/stationManageService'], function (app) {
     var app = angular.module('app');     app.controller('stationManageCtrl', ['$scope','$sce','stationManage', function ($scope,$sce,stationManage) {
        ...
        //theadr
        $scope.thHeader=stationManage.getThead();
        //添加岗位 系统功能数据
        ...
        //初始化查询数据
        stationManage.getDataTable({param:{'query':ckopts}},'/postInfoManger/getDicLists')
            .then(function (data) {
        ...
        function getTable(){
            ...
            stationManage.getDataTable({param:{query:opts}},'/postInfoManger/postInfoList')
                .then(function (data) {
                    if(data.code==-1){
                        alert(data.message);
                        $scope.result = [];
 ...
 //           code:'',
        ...
        $scope.enterAddStation= function () {
            ...
            $scope.stationModel.postType=1;
            stationManage.getDataTable({param:{query:$scope.stationModel}},$scope.stationTitle=='添加岗位'?'/postInfoManger/addPost':'/postInfoManger/updatePost')
                .then(function (data) {
...
//                code:item.code,
        ...
        $scope.storageDeleteCall= function (i,item) {
            if(confirm('确定删除吗?')){
                stationManage.getDataTable({param:{query:{id:item.id}}},'/postInfoManger/deletePost')
                    .then(function (data) {
                        alert(data.status.msg);
                        if(data.status.code=="0000"){
                            getTable();
        ...
        $scope.staFreeze= function (i,item) {
            if(confirm('确定冻结该岗位?')) {
            	stationManage.getDataTable({param:{query:{id:item.id,status:0}}}, '/postInfoManger/updatePostStatus')
                    .then(function (data) {
                        if(data.status.code=="0000"){
                        	 getTable();
                        }
        ...
        $scope.staResume= function (i,item) {
            if(confirm('确定恢复该岗位?')) {
            	stationManage.getDataTable({param:{query:{id:item.id,status:-1}}}, '/postInfoManger/updatePostStatus')
                    .then(function (data) {
                        if(data.status.code=="0000"){
                        	 getTable();
                        }

 ...
 */
'use strict';
define(['../../../app', '../../../services/storage/storage/confirmationCheckService'], function (app) {
     var app = angular.module('app');     app.controller('confirmationCheckCtrl', ['$scope', '$state', '$stateParams', '$sce', 'confirmationCheck','$window', function ($scope, $state, $stateParams, $sce, confirmation,$window) {
        $scope.banner = {};
        $scope.sku = '';
        //table头
        $scope.thHeader = confirmation.getThead();
        $scope.allTHeader = confirmation.getAllTHeader();
        ...
        $scope.alloModalCall= function (i,item) {
            confirmation.getDataTable({param: {query: {
                sku:item.sku,
                taskId:item.taskId,
                ckId:item.ckId
            }}},'/ckTaskPut/viewHasDonePutGoodsList')
                .then(function (data) {
        ...
        $scope.goodsAllocationHeader = [
            ...
            {field: 'putCount', name: '数量'/*, input: true, type: 'update'*/, style: 'width:50px;'},
            /*{
        ...
        function _initSelectCall() {
            ...
            $scope.goodsAlloSelectSet.select2Call = function () {
                ...
                if ($scope.goodsAlloSelectSet.select2Model != -1) {
                    var promise = confirmation.getDataTable({param: {query: {areaId: $scope.goodsAlloSelectSet.select2Model}}}, '/ckTaskPut/gethuoWeiNo');
                    promise.then(function (data) {

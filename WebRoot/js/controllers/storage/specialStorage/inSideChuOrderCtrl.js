...
define(['../../../app','../../../services/storage/specialStorage/inSideChuOrderService'], function (app) {
     var app = angular.module('app');
    app.controller('inSideChuOrderCtrl', ['$rootScope','$scope', '$sce','$filter', '$timeout', 'inSideChuOrder','$stateParams', function ($rootScope,$scope, $sce,$filter, $timeout, inSideChuOrder,$stateParams) {
        ...
        //theadr
        $scope.thHeader = inSideChuOrder.getThead();
        $scope.allTHeader = inSideChuOrder.getAllTHeader();
        $scope.compileTHeader = inSideChuOrder.compileTHeader();
        //分页下拉框
        $scope.pagingSelect = [
            {value: 5, text: 5},
        ...
        var pmsQuery = inSideChuOrder.getQuery();
        ...
        function get(){
            ...
            var promise = inSideChuOrder.getDataTable('/ckInsideChuOrder/ckInsideChuOrderList',{param: {query:opts}});
        ...
        $scope.checkDetail= function (i,item) {
            inSideChuOrder.getDataTable('/ckInsideChuOrder/ckInsideChuOrderDetailView',{param: {query: {
                    taskId:item.taskId
                }}})
                .then(function (data) {
        ...
        $scope.compileData= function (i,item) {
            inSideChuOrder.getDataTable('/ckInsideChuOrder/editCkInsideChuOrder',{param: {query: {
                    taskId:item.taskId
                }}})
                .then(function (data) {
                    if(data.status.code=="0000") {
        ...
        $scope.delete= function (i,item) {
            inSideChuOrder.getDataTable('/ckInsideChuOrder/deleteCkInsideChuOrderDetail',{param: {query: {
                id:item.detailId
            }}})
                .then(function (data) {
                    if(data.status.code=="0000") {
        ...
        $scope.confirmCompile= function (item) {
            ...
            inSideChuOrder.getDataTable('/ckInsideChuOrder/createOrEditCkInsideChuOrder',sendParams)
                .then(function (data) {
                    alert(data.status.msg)
                    if (data.status.code == "0000") {
        ...
        $scope.print=function(i,item){
            window.open("/print/inSideChuOrderPrint.html?tokenId="+$rootScope.userInfo.token+"&sessionid="+$rootScope.userInfo.sessionId+"&taskId="+ item.taskId);
        }
    }]);
});
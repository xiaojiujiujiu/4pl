 ...
 */
define(['../../../app','../../../services/storage/specialStorage/newOtherChuOrderService'], function (app) {
     var app = angular.module('app');
    app.controller('newOtherChuOrderCtrl', ['$rootScope', '$scope','$timeout', '$state', '$sce', '$filter', 'HOST', '$window','newOtherChuOrder',
        function ($rootScope, $scope,$timeout, $state, $sce, $filter, HOST, $window,newOtherChuOrder) {
            $scope.querySeting = {
                items: [ {
                    type: 'select',
                    model: 'typeList',
            ...
            //table头
            $scope.thHeader = newOtherChuOrder.getThead();
            $scope.newHeader = newOtherChuOrder.getThead1();
            ...
            var pmsQuery = newOtherChuOrder.getSearch();
            ...
            function get() {
                ...
                opts.isChu=$scope.searchModel.isChu;
                var promise = newOtherChuOrder.getDataTable(
                    '/ckOtherChuRuOrder/getCkGoodsList',//请求表格的数据接口
                    {
                        param: {
                            query: opts
                        }
            ...
            $scope.confirmImpor=function(){
                ...
                newOtherChuOrder.getDataTable('/ckOtherChuRuOrder/createOrEditCkOtherChuRuOrder', {param: opts})
                    .then(function (data) {
                        if(data.status.code=="0000") {
                            alert(data.status.msg, function () {
                                $state.go('main.otherChuRuOrder');
                            });
                        }else
